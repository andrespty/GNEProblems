...
class A10eU:
    ...
    @staticmethod
    def define_players():
        player_vector_sizes = [A10eU.P for _ in range(A10eU.N)]
        player_objective_functions = [0 for _ in range(A10eU.N)]  # change to all 0s

        f_player_constraints = [[0] for _ in range(A10eU.F)]
        c_player_constraints = [[1] for _ in range(A10eU.C)]
        player_constraints = f_player_constraints + c_player_constraints + [[2,3]]
        return [player_vector_sizes, player_objective_functions, player_constraints]

    @staticmethod
    def objective_functions():
        return [A10eU.obj_func]

    @staticmethod
    def objective_function_derivatives():
        return [A10eU.obj_der]

    @staticmethod
    def constraints():
        return [A10eU.g0, A10eU.g1, A10eU.g2, A10eU.g3, A10eU.g4]

    @staticmethod
    def constraint_derivatives():
        return [A10eU.g0_der, A10eU.g1_der, A10eU.g2_der, A10eU.g3_der, A10eU.g4_der]
    ...
    def utility_function_1(x_i, a, b, i):
        ...
        i: integer
        """
        return np.sum((a + i + A10eU.F) * np.log(x_i + b + 2 * (i + A10eU.F)), axis=0)
    ...
    def utility_function_2(x_i, c, d, i):
        ...
        i: integer
        """
        return np.sum((c + i + A10eU.F) * np.log(x_i + d + i + A10eU.F), axis=0)

    @staticmethod
    def obj_func_firms(x):
        p = x[-1].reshape(-1,1)
        y = np.hstack(x[:A10eU.F]).reshape(A10eU.P, -1)
        return (p.T @ y).reshape(-1,1) # returns (1,F) vector

    @staticmethod
    def obj_func_consumers(x):
        x = np.hstack(x[A10eU.F: A10eU.F + A10eU.C]).reshape(A10eU.P, -1)
        a, b, c, d = A10eU.get_constants()
        obj1 = []
        obj2 = []
        for i in range(int(A10eU.C / 2)):
            u1 = A10eU.utility_function_1(x[:, i].reshape(-1, 1), a, b, i + 1)
            u2 = A10eU.utility_function_2(x[:, i + int(A10eU.C / 2)].reshape(-1, 1), c, d, i + 1 + int(A10eU.C / 2))
            obj1.append(u1)
            obj2.append(u2)
    ...
    @staticmethod
    def obj_func(x):
        firms = A10eU.obj_func_firms(x)
        consumers = A10eU.obj_func_consumers(x)
        market = A10eU.obj_func_market(x)
    ...
    def utility_function_1_der(x_i, a, b, i):
        return (a + i + A10eU.F) / ((x_i + b + 2 * (i + A10eU.F)) * np.log(10) )

    @staticmethod
    def utility_function_2_der(x_i, c, d, i):
        return (c + i + A10eU.F) / ((x_i + d + i + A10eU.F) * np.log(10))
    ...
    def obj_func_consumers_der(x):
        ...
        der_10_20 = []
        for i in range(int(A10eU.C/2)):
            # print(i, i+ int(A10eU.C/2) )
            p1_10 = A10eU.utility_function_1_der(x[:,i].reshape(-1,1), a, b, i)
            p10_20 = A10eU.utility_function_2_der(x[:,i+int(A10eU.C/2)].reshape(-1,1), c, d, i+ int(A10eU.C/2))
    ...
    def obj_func_firms_der(x):
        p = np.tile(x[-1].reshape(-1,1), A10eU.F).reshape(-1,1)
    ...
    @staticmethod
    def obj_der(x):
        firms_der = A10eU.obj_func_firms_der(x) # (F*P, 1)
        consumers_der = A10eU.obj_func_consumers_der(x) # (C*P, 1)
        market_der = A10eU.obj_func_market_der(x) # (P, 1)
        return np.vstack((firms_der, consumers_der, market_der)).reshape(-1,1)


    @staticmethod
    def g0(x):
        idx = 10*np.array([i+1 for i in range(A10eU.F)]).reshape(-1,1)
        y = np.hstack(x[:A10eU.F]).reshape(A10eU.P, -1)**2
    ...
    def g1_der(x):
        ...
        zeros[A10eU.F*A10eU.P : A10eU.F*A10eU.P + A10eU.P * A10eU.C] = p_stack
        return zeros
    ...
    def get_xi():
        ...
        xi1s = np.hstack([xi1 for _ in range(int(A10eU.C/2))]).reshape(A10eU.P,-1)
        xi2s = np.hstack([xi2 for _ in range(int(A10eU.C/2))]).reshape(A10eU.P,-1)
    ...
    def get_constants():
        ...
        return a,b,c,d
#
# vector_sizes = A10eU.define_players()[0]
# x = [0 for _ in range(A10eU.N * A10eU.P)]
# actions = construct_vectors(x, vector_sizes)
# print(A10eU.obj_der(actions).shape)
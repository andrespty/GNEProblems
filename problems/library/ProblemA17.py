...
class ProblemA17(BaseProblem):
    ...
    def objectives(self):
        def obj_func_1(x: VectorList) -> jnp.ndarray:
            x1 = x[0]
            x2 = x[1]
            return x1[0]**2 + x1[0] * x1[1] + x1[1]**2 + jnp.sum(x1) * x2[0] - 25*x1[0] - 38*x1[1]

        def obj_func_2(x: VectorList) -> jnp.ndarray:
            x1 = x[0]
            x2 = x[1]
            return x2[0]**2 + jnp.sum(x1) * x2[0] - 25 * x2[0]
    ...
    def constraints(self):
        def g0(x: VectorList):
            x1 = x[0]
            x2 = x[1]
            return x1[0] + 2*x1[1] - x2[0] -14

        def g1(x: VectorList):
            x1 = x[0]
            x2 = x[1]
            return 3*x1[0] + 2*x1[1] + x2[0] - 30
        return [g0, g1]

...
class ProblemA18(BaseProblem):
    ...
    def objectives(self):
        def obj_func_1(x: VectorList) -> jnp.ndarray:
            x1 = x[0]
            x2 = x[1]
            return (15-self.S1(x)) * (x1[0] + x1[3]) + (15-self.S2(x)) * (x1[1] + x1[4]) + (15-self.S3(x)) * (x1[2] + x1[5])

        def obj_func_2(x: VectorList) -> jnp.ndarray:
            x1 = x[0]
            x2 = x[1]
            return (15-self.S1(x)) * (x2[0] + x2[3]) + (15-self.S2(x)) * (x2[1] + x2[4]) + (15-self.S3(x)) * (x2[2] + x2[5])
    ...
    def constraints(self):
        ...
        def g4(x: VectorList):
            return jnp.abs(self.S1(x) - self.S2(x)) - 1

        def g5(x: VectorList):
            return jnp.abs(self.S1(x) - self.S3(x)) - 1

        def g6(x: VectorList):
            return jnp.abs(self.S2(x) - self.S3(x)) - 1

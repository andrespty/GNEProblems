...
class A10cU:
    ...
    @staticmethod
    def define_players():
        player_vector_sizes = [A10cU.P for _ in range(A10cU.N)]
        player_objective_functions = [0 for _ in range(A10cU.N)]  # change to all 0s
        f_player_constraints = [[0] for _ in range(A10cU.F)]
        c_player_constraints = [[1] for _ in range(A10cU.C)]
        player_constraints = f_player_constraints + c_player_constraints + [[2,3]]
        return [player_vector_sizes, player_objective_functions, player_constraints]

    @staticmethod
    def objective_functions():
        return [A10cU.obj_func_firms,
                A10cU.obj_func_market
        ]

    @staticmethod
    def objective_function_derivatives():
        return [A10cU.obj_der]

    @staticmethod
    def constraints():
        return [A10cU.g0, A10cU.g1, A10cU.g2, A10cU.g3, A10cU.g4]

    @staticmethod
    def constraint_derivatives():
        return [A10cU.g0_der, A10cU.g1_der, A10cU.g2_der, A10cU.g3_der, A10cU.g4_der]
    ...
    def obj_func_firms(x):
        p = x[-1].reshape(-1,1)
        y = np.hstack(x[:A10cU.F]).reshape(A10cU.P, -1)
        return (p.T @ y).reshape(-1,1) # returns (1,F) vector

    @staticmethod
    def obj_func_consumers(x):
        x = np.hstack(x[A10cU.F: A10cU.F + A10cU.C]).reshape(A10cU.P, -1)
        obj = []
        for i in range(A10cU.C):
            Q_i = A10cU.get_constants(i=i + 1).get('Q_i')
            b_i = A10cU.get_constants(i=i + 1).get('b_i')
            u = -A10cU.utility_function(x[:, i].reshape(-1, 1), Q_i, b_i)
            obj.append(u)
    ...
    @staticmethod
    def obj_func(x):
        firms = A10cU.obj_func_firms(x)
        consumers = A10cU.obj_func_consumers(x)
        market = A10cU.obj_func_market(x)
    ...
    def obj_func_consumers_der(x):
        ...
        der_1_15 = []
        der_15_30 = []
        for i in range(int(A10cU.C/2)):
            A, b_i = A10cU.get_constants(i+1)
            B, bb_i = A10cU.get_constants(i+1 + int(A10cU.C/2))
            p1_15 = A10cU.utility_function_der(x[:,i].reshape(-1,1), A, b_i)
            p15_30 = A10cU.utility_function_der(x[:, i+ int(A10cU.C/2)].reshape(-1,1), B, bb_i)
    ...
    def obj_func_firms_der(x):
        p = np.tile(x[-1].reshape(-1,1), A10cU.F).reshape(-1,1)
    ...
    @staticmethod
    def obj_der(x):
        firms_der = A10cU.obj_func_firms_der(x) # (F*P, 1)
        consumers_der = A10cU.obj_func_consumers_der(x) # (C*P, 1)
        market_der = A10cU.obj_func_market_der(x) # (P, 1)
        return np.vstack((firms_der, consumers_der, market_der)).reshape(-1,1)


    @staticmethod
    def g0(x):
        idx = 10*np.array([i+1 for i in range(A10cU.F)]).reshape(-1,1)
        y = np.hstack(x[:A10cU.F]).reshape(A10cU.P, -1)**2
    ...
    @staticmethod
    def g0_der(x):
        y = 2 * x[:A10cU.F*A10cU.P].reshape(-1, 1)
        pad = np.array([0 for i in range(A10cU.C * A10cU.P + A10cU.P)]).reshape(-1, 1)
        return np.vstack((y, pad))

    @staticmethod
    def g1_der(x):
        zeros = np.zeros_like(x).reshape(-1,1)
        p = x[-A10cU.P:].reshape(-1, 1)
        p_stack = np.vstack([p for _ in range(A10cU.C)])
        zeros[A10cU.F*A10cU.P : A10cU.F*A10cU.P + A10cU.P * A10cU.C] = p_stack
        return zeros
    ...
    def get_xi():
        ...
        xi1s = np.hstack([xi1 for _ in range(int(A10cU.C/2))]).reshape(A10cU.P,-1)
        xi2s = np.hstack([xi2 for _ in range(int(A10cU.C/2))]).reshape(A10cU.P,-1)
    ...
    def get_constants(i=1):
        if i <= A10cU.C:
            ...
            b_i = np.array([50 + i + A10cU.F, 60 + i + A10cU.F, 70 + i + A10cU.F, 60 + i + A10cU.F, 60 + i + A10cU.F, 50 + i + A10cU.F]).reshape(-1, 1)
